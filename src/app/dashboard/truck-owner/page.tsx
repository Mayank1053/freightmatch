...
import { RatingModal } from '@/components/rating/rating-modal'; // Added RatingModal import for proper rating functionality
import {
  Truck,
  ArrowLeft,
  Search,
...
export default function BookingsPage() {
  ...
  const [showRatingModal, setShowRatingModal] = useState(false); // Replaced simple rating state with RatingModal state
  ...
  const handleAcceptBooking = (bookingId: string) => {
    ...
    toast({
      ...
      description:
        'You have successfully accepted the booking request. The shipper has been notified.',
    });
  };
  ...
  const handleDeclineBooking = (bookingId: string) => {
    ...
    toast({
      ...
      description:
        'The booking request has been declined. The shipper has been notified.',
      variant: 'destructive',
    });
  };
  ...
  const handleRateShipper = (booking: any) => {
    setSelectedBooking(booking);
    setShowRatingModal(true); // Use RatingModal instead of simple rating dialog
  };
  ...
  const handleRatingSubmit = (ratingData: any) => {
    // Replaced submitRating with proper rating submission handler
    if (!selectedBooking) return;

    // Update the booking with rating status (in real app, this would be an API call)
    setBookings((prev) =>
      prev.map((booking) =>
        booking.id === selectedBooking.id
          ? { ...booking, hasRated: true, rating: ratingData }
          : booking,
      ),
    );
    ...
    toast({
      ...
      description: `You successfully rated ${selectedBooking.shipper.name}. Thank you for your feedback!`,
    });
  ...
  return (
    <ProtectedRoute allowedRoles={['truck-owner']}>
      <div className='space-y-6'>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div>
            ...
            <p className='text-slate-600'>
              Manage your booking requests and confirmations
            </p>
          </div>
        </div>
              ...
              <CardHeader>
                <div className='flex items-start justify-between'>
                  <div className='space-y-1'>
                    ...
                    <CardDescription>
                      Requested on {booking.requestedAt} • Travel Date:{' '}
                      {booking.date}
                    </CardDescription>
                  </div>
                ...
                <div className='flex items-center justify-between p-4 bg-slate-50 rounded-lg'>
                  ...
                  <div className='flex gap-2'>
                    <Button
                      variant='outline'
                      size='sm'
                      onClick={() => handleCall(booking.shipper.phone)}>
        ...
        <Dialog open={showInvoice} onOpenChange={setShowInvoice}>
          <DialogContent className='max-w-2xl'>
            <DialogHeader>
              ...
              <DialogDescription>
                Invoice for booking #{selectedBooking?.id}
              </DialogDescription>
            </DialogHeader>
                ...
                <div className='flex justify-between items-start p-6 bg-slate-50 rounded-lg'>
                  <div>
                    <h3 className='text-lg font-semibold'>
                      FreightMatch Invoice
                    </h3>
                    <p className='text-sm text-slate-600'>
                      Invoice #INV-{selectedBooking.id}-2024
                    </p>
                    <p className='text-sm text-slate-600'>
                  ...
                  <div>
                    ...
                    <div className='space-y-1 text-sm'>
                      ...
                      <p>
                        ...
                        {selectedBooking.shipper.company}
                      </p>
                      <p>
                        <span className='text-slate-600'>Phone:</span>{' '}
                        {selectedBooking.shipper.phone}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {/* Rating Modal - CHANGE: Replaced simple rating dialog with comprehensive RatingModal */}
        {selectedBooking && (
          <RatingModal
            ...
            shipmentDetails={{
              ...
              trucker: selectedBooking.shipper.name,
              company: selectedBooking.shipper.company,
              amount: selectedBooking.amount,
              deliveryDate: selectedBooking.date,
            }}
            onRatingSubmit={handleRatingSubmit}
            userType='truck-owner' // Added userType prop for truck-owner specific rating categories
          />
        )}
        ...
        <Dialog open={showTripDetails} onOpenChange={setShowTripDetails}>
          <DialogContent className='max-w-2xl'>
            <DialogHeader>
              ...
              <DialogDescription>
                Complete information about your confirmed booking
              </DialogDescription>
            </DialogHeader>
        ...
        <Dialog open={showStartTrip} onOpenChange={setShowStartTrip}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Are you ready to start this trip? This will notify the shipper
                and begin tracking.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className='space-y-4'>
                <div className='p-4 bg-green-50 rounded-lg'>
                  ...
                  <div className='text-sm text-green-700'>
                    ...
                    <p>
                      Cargo: {selectedBooking.cargo.type} (
                      {selectedBooking.cargo.weight})
                    </p>
                ...
                <div className='p-4 bg-blue-50 rounded-lg'>
                  <h4 className='font-medium text-blue-900 mb-2'>
                    What happens when you start the trip:
                  </h4>
                  <ul className='text-sm text-blue-700 space-y-1'>
                    <li>
                      • Shipper will be notified that the trip has started
                    </li>
                    <li>• Live tracking will begin for the shipper</li>
                    <li>• Trip status will change to "In Transit"</li>
                    <li>
                      • You can update progress and communicate with shipper
                    </li>
                  </ul>
                </div>
        ...
        <Dialog open={showCompleteTrip} onOpenChange={setShowCompleteTrip}>
          <DialogContent>
            <DialogHeader>
              ...
              <div className='space-y-4'>
                ...
                <div className='flex gap-3'>
                  <Button
                    variant='outline'
                    className='flex-1 bg-transparent'
                    onClick={() => setShowCompleteTrip(false)}>
                    Cancel
                  </Button>
                  <Button
                    className='flex-1 bg-green-600 hover:bg-green-700'
